...
- [ ] **Copy proto files and create gen script** — create `mind_mcp/proto/`, copy `breath_sessions.proto` and `auth.proto` from `mind_api/proto/`; create `scripts/gen_proto.sh` that runs `protoc --plugin=ts-proto --ts_proto_out=src/generated/ proto/*.proto`; add `proto:gen` script to `package.json` and document in `CLAUDE.md`
